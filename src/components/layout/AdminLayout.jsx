...
export default function AdminLayout() {
    ...
    // 2. Các hàm điều khiển y hệt Vue
    const toggleSidebar = () => {
    ...
    return (
        <div className="flex min-h-screen bg-transparent transition-colors duration-300 selection:bg-orange-500/20 overflow-x-hidden">

            <div className="fixed inset-0 z-0 pointer-events-none trong-dong-pattern"></div>

            {/* Lớp Overlay y hệt Vue (<Transition name="fade">) */}
            <AnimatePresence>
            ...
            >
                ...
                <button onClick={() => setIsSidebarOpen(false)} className="md:hidden absolute right-4 top-6 apple-btn-icon !bg-transparent !text-gray-500 hover:!text-[#ff3b30] z-50">
                ...
                <div className={`flex items-center justify-center px-2 mb-8 pt-safe overflow-hidden transition-all duration-300 ${isSidebarOpen ? 'h-20' : 'h-14'}`}>
                    <div className={`logo-custom transition-all duration-300 ${isSidebarOpen ? 'w-44 h-full' : 'w-12 h-12'}`} style={{ backgroundImage: `url(${logoImg})` }}></div>
                </div>

                <nav className="flex-1 space-y-1.5 mt-2 overflow-y-auto custom-scrollbar pr-1">
                    <NavItem to="/" icon={Home} label="Trang chủ" isOpen={isSidebarOpen} onClick={closeOnMobile} exact />
            ...
            {/* MAIN CONTENT y hệt class của Vue */}
            <main className={`flex-1 w-full max-w-full flex flex-col min-h-screen relative z-10 transition-all duration-300 ${isSidebarOpen ? 'md:ml-64' : 'md:ml-[88px]'}`}>
                ...
                <div className="md:hidden sticky top-0 z-40 flex items-center justify-between px-4 py-3 pt-safe apple-glass !rounded-none !border-x-0 !border-t-0">
                    <div className="flex items-center h-12">
                        <div className="logo-custom w-36 h-full" style={{ backgroundImage: `url(${logoImg})` }}></div>
                    </div>
                    ...
                    <div className="flex-1 relative w-full mt-4 md:mt-0">
                        ...
                        <div className={`h-full transition-all duration-700 ease-in-out ${isLocked ? 'blur-[10px] pointer-events-none select-none opacity-30 grayscale-[30%]' : 'blur-0 opacity-100'}`}>
                            <AnimatePresence mode="wait">
                                <motion.div key={location.pathname} initial={{ opacity: 0, y: 10, scale: 0.99 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.99 }} transition={{ duration: 0.2 }} className="h-full">
                                    <Outlet />
                                </motion.div>
                            </AnimatePresence>
                        </div>
                            ...
                            {isLocked && (
                                <motion.div initial={{ opacity: 0, scale: 0.8, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 10 }} transition={{ type: "spring", stiffness: 300, damping: 25 }} className="absolute inset-0 z-50 flex items-center justify-center p-4">
                                    <div className="apple-glass p-8 rounded-[32px] w-full max-w-[340px] flex flex-col items-center shadow-2xl border border-[var(--border-subtle)] relative">
                                        ...
                                        <form onSubmit={handleUnlock} className="w-full relative z-10 mt-5">
...
// COMPONENT NAV ITEM TẠO THEO CHUẨN CỦA VUE
